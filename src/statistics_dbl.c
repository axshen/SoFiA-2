...
///                                                                      ///
/// SoFiA 2.3.1 (statistics_dbl.c) - Source Finding Application          ///
...
double kin_maj_axis_dbl(const double *centroidX, const double *centroidY, const double *sum, const size_t size, const size_t first, const size_t last)
{
	...
	double fullAngle = atan2(centroidY[last] - centroidY[first], centroidX[last] - centroidX[first]);
	
	// Correct for full angle and astronomers' favourite definition of PA:
	double difference = fabs(atan2(sin(fullAngle) * cos(pa) - cos(fullAngle) * sin(pa), cos(fullAngle) * cos(pa) + sin(fullAngle) * sin(pa)));
